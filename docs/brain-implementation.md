...
| **Operational data as OKF** (§4.1, decisions #11/#12) — *shipped* | `brain.sync_schema` materializes OKF `type: table` schema docs (columns from the live DB) for the core operational tables + every `<module>__*` table into `50-schema/`, indexed; `Module.dataSchema` lets a module enrich its descriptions (auto-discovery is the baseline). `brain.ingest_rows` indexes operational + module rows as by-reference row pointers (snippet only). `brain.ask` grounds on both — schema catalog for column-aware SQL, row pointers for live data + citations. Seeded boot/tenant-create schema sync + an hourly ingest routine. Verified by a live faked-email + `demo__widgets` E2E (12/12). |
